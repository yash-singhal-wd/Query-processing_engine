...
void BufferManager::writePage1(string tableName, string pageIndex, vector<vector<int>> rows, int rowCount)
{
    logger.log("BufferManager::writePage");
    Page page(tableName, pageIndex, rows, rowCount);
    page.pageName = "../data/temp/" + page.tableName +"/" + page.tableName + "_Page" + (pageIndex);
    page.writePage();
}

void BufferManager::writePage_final_wave(string tableName, string pageIndex, vector<vector<int>> rows, int rowCount)
{
    logger.log("BufferManager::writePage");
    Page page(tableName, pageIndex, rows, rowCount);
    page.pageName = "../data/temp/" + page.tableName + "_Page" + (pageIndex);
    page.writePage();
}

void BufferManager::writePage_customFolder(string folder, string tableName, string pageIndex, vector<vector<int>> rows, int rowCount)
{
    logger.log("BufferManager::writePage");
    Page page(tableName, pageIndex, rows, rowCount);
    page.pageName = "../data/temp/" + folder + "/" + page.tableName + "_Page" + (pageIndex);
    page.writePage();
}
/**
